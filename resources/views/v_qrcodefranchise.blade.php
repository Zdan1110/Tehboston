...
<body class="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-cyan-200 to-teal-600 p-6">
  ...
  <div class="bg-white rounded-lg shadow-lg p-8 w-full max-w-md relative z-10">
    ...
    <h1 class="text-3xl font-extrabold text-teal-700 mb-8 text-center">QR Code Status Franchise Baru</h1>
    @if (session('success'))
      <div 
          x-data="{ show: true }" 
    ...
    <form class="flex flex-col space-y-6">
        ...
        <div class="flex justify-center">
          <img src="{{ asset('uploads/qrcode/' . $idcalon->id_franchisebaru . '.png') }}" alt="QR Code">
        </div>
        <h2 class="text-2xl font-extrabold text-teal-700 mb-8 text-center">ID Daftar Franchise : {{ $idcalon->id_franchisebaru }}</h2>
      <!-- Tombol Lihat Status -->
        <a
            href="{{ route('download.qrcodefranchise', ['id_franchisebaru' => $idcalon->id_franchisebaru]) }}"
