...
<body class="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-6">
  ...
  <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
    <p class="mb-3 text-gray-700"><strong>ID:</strong> {{ $calon->id_calon }}</p>
    <p class="mb-6 text-gray-700"><strong>Nama Lengkap:</strong> {{ $calon->nama_lengkap }}</p>

    <!-- Tracking container -->
    <div class="flex items-center justify-between space-x-4">
      <!-- Status Diterima -->
      <div class="flex flex-col items-center flex-1">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mb-2 {{ $calon->status == 'Diterima' ? 'pulse-icon' : '' }}"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
          viewBox="0 0 24 24"
      ...
      <!-- Status Proses -->
      <div class="flex flex-col items-center flex-1">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mb-2 {{ $calon->status == 'Proses' ? 'pulse-icon' : '' }}"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
          viewBox="0 0 24 24"
          ...
          stroke-linejoin="round"
          style="
            color: {{ $calon->status == 'Proses' ? '#ca8a04' : '#9ca3af' }};
        ...
        <span class="text-sm font-semibold"
          style="
            color: {{ $calon->status == 'Proses' ? '#ca8a04' : '#6b7280' }};
      ...
      <!-- Status Ditolak -->
      <div class="flex flex-col items-center flex-1">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10 mb-2 {{ $calon->status == 'Ditolak' ? 'pulse-icon' : '' }}"
          fill="none"
          stroke="currentColor"
          stroke-width="2"
          viewBox="0 0 24 24"
  ...
  <a href="/home" 
     class="px-6 py-2 bg-teal-600 text-white rounded-md hover:bg-teal-700 transition-colors duration-200">
    Kembali
  </a>
  </div>
